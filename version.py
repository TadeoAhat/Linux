...
with open("example.cimg", "rb") as file: #rb je binarny mod na citanie a with zabezpeci ze sa subor VZDY zavrie po skonceni bloku
    ...
    #DOPLNAM KOD Z MINULEJ ULOHY
    if isinstance(meta, dict): #iba checknem ci je to slovnik, ak hej dostanem info
        version = meta.get("VersionInformation")    
        version_date = meta.get("VersionDate")     
        version_note = meta.get("VersionNote")      
    else:
        version = None
        version_date = None
        version_note = None